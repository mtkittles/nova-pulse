import { BarChart3, Eye, Gem } from "lucide-react"
...
const TIER: Record<RecommendationTier, { icon: typeof Gem; label: string; cls: string }> = {
  value: { icon: Gem, label: "Value", cls: "border border-[color:var(--cyan)] bg-[var(--cyan)] text-[color:var(--on-accent)]" },
  analysis: { icon: BarChart3, label: "Analiza", cls: "border border-[color:var(--cyan)] bg-transparent text-[color:var(--cyan)]" },
  watchlist: { icon: Eye, label: "Watchlist", cls: "border border-transparent bg-transparent text-[color:var(--cyan)]" },
}
